...
def _logistic_loss_and_grad(w, X, y, alpha, sample_weight=None):
    ...
    grad : ndarray of shape (n_features,) or (n_features + 1,)
        Logistic gradient.
    """
    # save_pickle("/home/thomas/Documents/consscortk/sqmnn/input_args.pkl", w, X, y, alpha)
    ...
    # save_pickle("/home/thomas/Documents/consscortk/sqmnn/output_args.pkl", out, grad)
    return out, grad
...
def _logistic_loss_and_grad_grouped_samples(w, X, y, alpha, X_class, sample_weight=None):
    ...
    _class_weights = minmax_scale((_class_counts.sum()/_class_counts)/(_class_counts.sum()/_class_counts).sum()) + 1
    for xc in _classes:
        out, grad = _logistic_loss_and_grad(w=w,
                                            X=X[X_class==xc],
                                            y=y[X_class==xc],
                                            alpha=alpha,
    ...
    return np.dot(outs, _class_weights), np.average(grads, axis=0, weights=_class_weights)
...
def _logistic_regression_path(X, y, pos_class=None, Cs=10, fit_intercept=True,
    ...
    if multi_class == 'multinomial':
        ...
        target = y_bin
        if solver == 'lbfgs':
            func = _logistic_loss_and_grad_grouped_samples
        elif solver == 'newton-cg':
            func = _logistic_loss
            def grad(x, *args): return _logistic_loss_and_grad_grouped_samples(x, *args)[1]
...
class LogisticRegressionGroupedSamples(BaseEstimator, LinearClassifierMixin,
                         SparseCoefMixin):
    """
    ...
    def fit(self, X, y, sample_weight=None, X_class=None):
        """
        ...
        fold_coefs_ = Parallel(n_jobs=self.n_jobs, verbose=self.verbose,
            ...
            path_func(X, y, pos_class=class_, Cs=[C_],
                      ...
                      sample_weight=sample_weight, X_class=X_class)
...
class LogisticRegressionCV(LogisticRegressionGroupedSamples, BaseEstimator,
