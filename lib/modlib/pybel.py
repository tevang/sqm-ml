...
Global variables:
  ...
  fps - a list of supported fingerprint_types
...
def _formatstodict(list):
    if sys.platform[:4] == "java":
        l = [list.get(i) for i in range(list.size())]
    else:
        l = []
    broken = [x.replace("[Read-only]", "").replace("[Write-only]","").split(" -- ") for x in l]
...
"""A list of supported fingerprint_types"""
...
class Molecule(object):
    ...
    def calcfp(self, fptype="FP2"):
        ...
        Optional parameters:
           fptype -- the fingerprint_type (default is "FP2"). See the
        ...
        try:
            ...
            raise ValueError("%s is not a recognised Open Babel fingerprint_type" % fptype)
