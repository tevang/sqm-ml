...
class Statistics(object):
    ...
    Along the first axis (which length is given by the *n* argument) are
...
class ParetoFront(HallOfFame):
    ...
    def update(self, population):
        ...
        """
        for ind in population:
            # print("DEBUG: checking individual", ind)
            is_dominated = False
            has_twin = False
            to_remove = []
            for i, hofer in enumerate(self):    # hofer = hall of famer
                # print("DEBUG: against hofer", hofer)
                # if isDominated(ind.fitness.wvalues, hofer.fitness.wvalues):
                R_self = get_R(ind.fitness.getValues(), ind.fitness.weights)
                R_other = get_R(hofer.fitness.getValues(), hofer.fitness.weights)
                # print("DEBUG: R_self=", R_self, "R_other=", R_other)
                # if isDominated_HyperEllipsoid(ind.fitness.getValues(), ind.fitness.weights, hofer.fitness.getValues(), hofer.fitness.weights):
                if R_self < R_other:
                    is_dominated = True
                    break
                # elif isDominated(hofer.fitness.wvalues, ind.fitness.wvalues):
                # if isDominated_HyperEllipsoid(hofer.fitness.getValues(), hofer.fitness.weights, ind.fitness.getValues(), ind.fitness.weights):
                elif R_self > R_other:
                    to_remove.append(i)
...
def cxOnePointMaxOnes(ind1, ind2, max_ones, max_iter):
    """Execute a one point crossover on the input individuals.
    The two individuals are modified in place. The resulting individuals will
    respectively have the length of the other.
    ...
    parent_ind2 = ind2
    iterations=0
    while (ind1.__eq__(parent_ind1) and ind2.__eq__(parent_ind2)):
        size = min(len(ind1), len(ind2))
        cxpoint = random.randint(1, size - 1)
        ind1[cxpoint:], ind2[cxpoint:] = ind2[cxpoint:], ind1[cxpoint:]
        
        if (ind1.count(1) > max_ones or ind2.count(1) > max_ones):
            print("DEBUG: ind1.count(1) > max_ones:",ind1.count(1), "or ind2.count(1) > max_ones:",ind2.count(1))
            ind1 = parent_ind1
            ind2 = parent_ind2
        
        iterations += 1
        if (iterations > max_iter): # if max iterations is exceeded return the original individuals without any cross-over
            break
...
def cxUniform(ind1, ind2, indpb, min_positives):
    """Modified functions to execute a uniform crossover that modify in place the two individuals.
    The attributes are swapped according to the *indpb* probability. The difference is that a specified minimum number of positive integers (scoring
    function indices) is imposed.
    
    :param ind1: The first individual participating in the crossover.
    :param ind2: The second individual participating in the crossover.
    :param indpb: Independent probabily for each attribute to be exchanged.
    :param min_positives:   minimum number of positive numbers in the individual.
    :returns: A tuple of two individuals.
    ...
    parent_ind1=ind1
    parent_ind2=ind2
    unique_positive_numbers1=0
    unique_positive_numbers2=0
    
    while (unique_positive_numbers1 < min_positives or unique_positive_numbers2 < min_positives):
        # do the cross-over
        ind1=parent_ind1
        ind2=parent_ind2
        for i in range(size):
            if random.random() < indpb:
                ind1[i], ind2[i] = ind2[i], ind1[i]
        # check if the minimum positive numbers condition is met for the 1st individual
        sorted_ind1=ind1
        sorted_ind1.sort()
        unique_positive_numbers1=0
        previous_num=sorted_ind1[0]
        if previous_num >= 0:
                unique_positive_numbers1+=1
        for indx in range(1,len(sorted_ind1)):
            current_num=sorted_ind1[indx]
            if (current_num != previous_num and current_num >=0):
                unique_positive_numbers1+=1
            previous_num=current_num
        # check if the minimum positive numbers condition is met for the 2nd individual
        sorted_ind2=ind2
        sorted_ind2.sort()
        unique_positive_numbers2=0
        previous_num=sorted_ind2[0]
        if previous_num >= 0:
                unique_positive_numbers2+=1
        for indx in range(1,len(sorted_ind2)):
            current_num=sorted_ind2[indx]
            if (current_num != previous_num and current_num >=0):
                unique_positive_numbers2+=1
            previous_num=current_num
...
def mutPolynomialBounded(individual, eta, low, up, indpb):
    """Modified version of the Polynomial mutation as implemented in original NSGA-II algorithm in
    C by Deb. Instead of rand = random.random() I use rand = random.uniform(0,1).
    ...
    :param up: A value or a sequence of values that is the upper bound of the
               search space.
    :param indpb: Probability for each attribute of the individual to be mutated.
    :returns: A tuple of one individual.
    """
    size = len(individual)
...
def mutFlipBitMaxOnes(individual, indpb, max_ones):
    ...
    :param max_ones: Maximum number of "1"s in the idividual object.
    :returns: A tuple of one individual.
    ...
    if(parent_individual.count(1)>max_ones):
        print("DEBUG: parent_individual.count(1)>max_ones:",parent_individual)
        sys.exit(2)
    random.seed()
    #print("DEBUG: parent_individual == individual:",parent_individual.__eq__(individual)
    #print("DEBUG: parent_individual:",parent_individual
    #print("DEBUG: individual:",individual
    while (parent_individual.__eq__(individual)):
        print("DEBUG: parent_individual == individual:",parent_individual.__eq__(individual))
        print("DEBUG: parent_individual:",parent_individual)
        print("DEBUG: individual:",individual)
        print("DEBUG: ones=",ones,"max_ones=",max_ones)
        if (ones<max_ones):
            indx=random.randint(0,len(individual)-1)
            if random.random() < indpb:
                individual[indx] = type(individual[indx])(not individual[indx])
            print("DEBUG: new individual:",individual)
            print("DEBUG: individual:", individual.count(1))
        ...
        if (ones>max_ones):
            print("DEBUG: ones>max_ones",ones,max_ones)
            individual=parent_individual
            ones=individual.count(1)
            print("DEBUG: parent_individual != individual:",parent_individual.__ne__(individual))
            print("DEBUG: parent_individual:",parent_individual)
            print("DEBUG: individual:",individual)
            #sys.exit(2)
...
def mutUniformIntThresholdPositives(individual, low, up, indpb, min_positives):
    """A modified mutUniformInt function that prevents the number of positive integers in the individual to fall below a threshold value.
    ...
    :param up: The upper bound of the range from wich to draw the new
                integer.
    :param indpb: Probability for each attribute to be mutated.
    :parame min_positives:  the minimum number of positive integers in the individual.
    :returns: A tuple of one individual.
    """
    ...
    size = len(individual)
    parent_individual=individual
    unique_positive_numbers=0
    
    while (unique_positive_numbers < min_positives):
        # mutate the individual
        individual=parent_individual
        for indx in range(len(individual)):
            if random.random() < indpb:
                individual[indx] = random.randint(low, up)
        # check if the minimum positive number condition is met
        sorted_individual=individual
        sorted_individual.sort()
        unique_positive_numbers=0
        previous_num=sorted_individual[0]
        if previous_num >= 0:
                unique_positive_numbers+=1
        for indx in range(1,len(sorted_individual)):
            current_num=sorted_individual[indx]
            if (current_num != previous_num and current_num >= 0):
                unique_positive_numbers+=1
            previous_num=current_num
...
def isDominated_HyperEllipsoid(values1, weights1, values2, weights2):
    """Returns wheter or not *wvalues1* dominates *wvalues2*.
    
    :param values1: The fitness values that would be dominated.
    :param values2: The fitness values of the dominant.
    :param weights1: The weights that would be dominated.
    :param weights2: The weights of the dominant.
    :returns: :obj:`True` if wvalues2 dominates wvalues1, :obj:`False`
              otherwise.
    """
    # print("DEBUG isDominated_HyperEllipsoid: values1=", values1, "weights1=", weights1, "values2=", values2, "weights2=", weights2)
    R_self, R_other = 0, 0
    for v1, w1, v2, w2 in zip(values1, weights1, values2, weights2):
        R_self += w1*(v1)**2
        R_other += w2*(v2)**2
    
    # print("DEBUG isDominated_HyperEllipsoid: R_self=", R_self, "R_other=", R_other)
    if R_self > R_other:
        return False
    else:
        return True

def get_R(values, weights):
    """Returns wheter or not *wvalues1* dominates *wvalues2*.
    ...
    :returns: :obj:`True` if wvalues2 dominates wvalues1, :obj:`False`
              otherwise.
    """
    # print("DEBUG get_R: values=", values, "weights=", weights)
    R = 0
